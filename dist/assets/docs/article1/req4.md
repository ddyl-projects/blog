...
- **Side_Menu**: The right-hand side navigation panel displaying the 10 latest articles, a home link, and an index link
...
- **Linked_Document**: A markdown file referenced via a relative link within an article, rendered inline in the content area with a back button to return to the referring article
...
4. THE Side_Menu SHALL display a "Home" link at the top of the menu that navigates to the Introduction_Page
5. WHEN a new article is added to the Article_Manifest, THE Side_Menu SHALL include the new article in the list on the next page load if it is among the 10 most recent articles by date
6. WHEN the screen width is less than 768px, THE Side_Menu SHALL collapse or reposition to maintain usability on smaller screens
...
2. THE header section SHALL display a logo image (from assets/images/logo.png) aligned to the left, with the header having a height of 150px and the logo scaled to fit within it
...
4. THE footer section SHALL remain at the bottom of the page and display "© [current year] Damian Dyl. All rights reserved."
...
1. THE Blog_Application SHALL provide a development mode (via npm script named "dev") that serves the site locally using Vite as the development server
...
### Requirement 13: Browser Caching Prevention

**User Story:** As a blog author, I want to ensure readers always see the latest content, so that updates are immediately visible without manual cache clearing.

#### Acceptance Criteria

1. THE Blog_Application SHALL include HTTP cache-control meta tags (Cache-Control: no-cache, no-store, must-revalidate; Pragma: no-cache; Expires: 0) in the HTML document head to prevent browsers from serving stale content

### Requirement 14: Linked Document Navigation

**User Story:** As a blog author, I want to link to other markdown files from within my articles and have them render inline with a back button, so that I can provide supplementary content without losing the reader's place.

#### Acceptance Criteria

1. WHEN an article contains a relative link to a .md file, THE Client_Renderer SHALL intercept the click and render the linked markdown file inline in the content area instead of navigating away
2. WHEN a Linked_Document is displayed, THE Blog_Application SHALL show a "← Back" button at the top of the content that returns the reader to the referring article
3. THE Blog_Application SHALL NOT intercept links to external .md files (those starting with http:// or https://)

### Requirement 15: Layout Stability

**User Story:** As a blog reader, I want the page layout to remain stable when navigating between pages, so that the content doesn't shift horizontally.

#### Acceptance Criteria

1. THE Blog_Application SHALL always display the vertical scrollbar (overflow-y: scroll on the html element) to prevent horizontal layout shifts when navigating between pages with different content heights

### Requirement 16: Non-Clickable Logo

**User Story:** As a blog reader, I want the header logo to be a static image without link behavior, so that I don't accidentally navigate away when clicking on it.

#### Acceptance Criteria

1. THE Blog_Application SHALL display the logo image in the header as a non-clickable static image element without any anchor tag or navigation behavior

### Requirement 17: Tag Filtering on Article Index
